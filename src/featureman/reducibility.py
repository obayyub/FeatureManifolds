...
def estimate_mutual_information(a, b, n_bins=40, clip_range=10):
    """
    ...
    hist_2d, _, _ = np.histogram2d(
        ...
        b_clipped,
        bins=n_bins,
        range=[[-clip_range, clip_range], [-clip_range, clip_range]],
    )
    ...
    p_ab = hist_2d / np.sum(hist_2d)  # joint probability
    p_a = np.sum(p_ab, axis=1)  # marginal of a
    p_b = np.sum(p_ab, axis=0)  # marginal of b
...
def compute_mixture_index_silent(points, epsilon=0.1, n_steps=200, lr=0.1):
    ...
    for step in range(n_steps):
        ...
        for i in range(n_samples):
            ...
            grad_v += (
                sigmoid_grad * (-sign_proj / (T + 1e-8)) * (points[i] / (rms + 1e-8))
            )
...
def test_irreducibility_on_projections_silent(
    full_reconstructions, cluster_reconstructions, pc_pairs=None, epsilon=0.1
):
    """
    ...
    pca = PCA()
    pca_transformed = pca.fit(full_reconstructions)

    # Determine which PC pairs to test
    if pc_pairs is None:
        pc_pairs = [(0, 3), (1, 4), (2, 5)]
        pc_pairs = [
            (i, j)
            for i, j in pc_pairs
            if i < len(pca.explained_variance_ratio_)
            and j < len(pca.explained_variance_ratio_)
            and pca.explained_variance_ratio_[i] > 0.01
            and pca.explained_variance_ratio_[j] > 0.01
        ]
    ...
    indices = np.where(np.isin(inputs[:, 0] + inputs[:, 2], targets))[0]

    for pc_i, pc_j in pc_pairs:
        # Extract 2D projection
        points_2d = pca.transform(cluster_reconstructions[indices])[:, [pc_i, pc_j]]
        ...
        mix_index, best_v, best_c = compute_mixture_index_silent(
            points_2d, epsilon=epsilon
        )
        ...
        results[(pc_i, pc_j)] = {
            ...
            "variance_explained": (
                pca.explained_variance_ratio_[pc_i],
...
def plot_and_save_projection(
    pc_pair, result, cluster_idx, save_dir="irreducibility_results"
):
    ...
    fig.suptitle(
        f"Cluster {cluster_idx} - PC{pc_pair[0]}-PC{pc_pair[1]} | S(f)={sep_idx:.3f}, M_ε(f)={mix_idx:.3f}",
        fontsize=14,
        fontweight="bold",
    )
...
def analyze_cluster_irreducibility_silent(
    full_reconstructions,
    cluster_reconstructions,
    cluster_idx,
    save_plots=True,
    save_dir="irreducibility_results",
):
    """
    Silent version that saves plots and returns summary data
    """
    # Test multiple 2D projections (no progress prints)
    pca, results = test_irreducibility_on_projections_silent(
        full_reconstructions, cluster_reconstructions
    )
...
def save_analysis_summary(all_summaries, save_dir="irreducibility_results"):
    ...
    for summary in all_summaries:
        row = {
            ...
            "pc1_variance": summary["pca_variance"][0]
            if len(summary["pca_variance"]) > 0
            else 0,
            "pc2_variance": summary["pca_variance"][1]
            if len(summary["pca_variance"]) > 1
            else 0,
            "pc3_variance": summary["pca_variance"][2]
            if len(summary["pca_variance"]) > 2
            else 0,
            "is_irreducible": summary["is_irreducible"],
            "irreducibility_score": summary["mean_separability"]
            * (1 - summary["mean_mixture"]),
        }
    ...
    print("🏆 Top 5 irreducible clusters:")
    print(
        df.head()[
            [
                "cluster_idx",
